...
HTML defines what buiding blocks are there in a web page and how they relate to each other. How many textboxes and buttons are there? Are they contained in the same box?
...
To check out what DOM looks like, you can inspect the web page like so:
...
We often want a web page dynamic and interactive -- that means the page needs to change according to users' input: adding or removing text here and there, showing a modal, or charting some new data retrieved from the server.

Remember, in order to change what's on a web page, we need to update the DOM. It's like that the artist won't be able to paint new portraits until Domo changes to a new pose.

How would we get Domo to change to a new pose?

We talk to him. More precisely, we talk into his ears, which happen to have a name: _DOM API_. We update DOM by writing JavaScript code that talks to DOM API.

![ JavaScript to instruct Domo to pose ](TODO)

## Problems

For quite a while, developers

In the past, a developer directly talks to Domo.
