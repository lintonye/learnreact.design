...
Let's start with the basics: how the web works, and what DOM is.

# How the web works

When you enter the address of your favorite website into a browser, your computer tries to contact another computer far away, commonly referred to as _server_. Then, the two computers start chit chatting. Typically your computer makes a request for some information and the server responds:

> Your computer: Yo, what's good about this random site learnreact.design?
> The server: Hang on, let me grab something for you. Beep. Bop.

The server's response includes three types of content (of course there are other types but let's keep it simple for now):

- HTML
- CSS
- JavaScript

Once your browser receives this information, which often looks like giberish to our human eyes, it decodes it and displays it as a web page, something that we could comprehend and enjoy: titles, paragraphs, layouts, icons, images and animations.

Let's zoom into the browser to see what's going on under the hood.

## What is DOM

HTML defines the buiding blocks of a web page and how they relate to each other. How many textboxes and buttons are there? Are they contained in the same box? The browser uses this information to construct something called DOM.

![ DOM is born ](TODO)

Wait a second, DOM is a ... tree? Yup a tree! Oddly enough, a lot of things in your computer look like a tree. Let's give our tree friend a nickname... ummm what about Domo?
...
What's JavaScript's role in this picture? It's like

JavaScript to make change of Domo's pose

What's CSS's role in this picture?

What about JavaScript?

jQuery or React are known as libraries and they are the tools a developer uses to communicate with Domo.

The browser takes these information and constructs DOM.

[ zoom into browser ]

- computer connects to server, computers have conversations
- decode server's talks: html/CSS/JS
- zoom into browser:
  - Domo is born out of HTML
  - he's a model

[DOMo the model, the artist draws control on the screen ]

To check out what DOM looks like, you can inspect in the browser.

[ inspect ]

Hint: You can inspect this very page too.

[ put a DOMO ascii art in the console ]

JavaScript is how the browser takes user input, engages further conversations with the server and maybe other servers, and instructs Domo what poses he needs to do (and display that result on the web page).

The developer uses JavaScript to instruct Domo what to do.

[ JavaScript to instruct Domo to pose ]

# Problems

But it can be tedious to directly talk to Domo, because:

- code is large, complex and difficult to manage

  - React tries to make it easy to build complicated apps by making it easy to split up your app into many simple, independent components. This means that when you change code in one place, other parts of your app don't break.

- it's slow
- Domos in different browsers might speak slightly different languages
- TODO why react?
  ...
  - by definition, imperative code relies on the context of the current state.
  - UI state hard to manage: need to remember the current state, need to remember what changed
