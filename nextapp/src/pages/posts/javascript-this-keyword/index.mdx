...
This article assumes basic understanding of JavaScript. You'll also find it useful if you are familiar with other languages such as Java, Python or Ruby.
...
In this code, we get the default value of `this`, the `Window` object (in fact the default value could be `undefined`, but I'll cover it [later](TODO)).
...
As I mentioned, the default value of `this` isn't always the `Window` object. That's because of the so-called "strict mode".
...
There's also a strict mode that affects the default value of `this`.
...
If we want to use the value of `this` inside the second callback function, we'd have to store its value to a variable in the outer function.

This actually becomes a pattern:
...
The answer is NO! You can't even call an arrow function with `new`.
...
Alright, the above is how `this` works in JavaScript. Does it still feel weird to you? IMO it feels much better if we understand the rules behind. Here's a recap:
