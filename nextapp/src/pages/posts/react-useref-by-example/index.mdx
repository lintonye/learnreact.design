...
`useRef` is a built-in Hook of React. Perhaps you have used it to access DOM nodes (If not, don't worry. I'll cover it in this post).

But do you know that it can do way more?

This is a deep dive into `useRef`. We'll study this little Hook in an example-driven approach:
...
Along the way, we'll build a stop watch and a like button (yes, exactly the same like button on this blog, feel free to steal the code):
