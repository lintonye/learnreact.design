...
<img src={imgInspect} alt="Inspect in Chrome" />
...
Imperative programming emerged from the day when the computers were primitive. People had to instruct them in detail: where to store the numbers, how to multiply etc. But this eventally got unmanageable, people wrote smart software that convert definition of problems into detailed instructions. Declarative programming was born.
...
Now let's review what we have learned so far and get our hands on a few real React components. To make it easy for you to understand, I left out some details in the code (notebly JavaScript). The goal is to let the core ideas shine through. If you are confident in reading JavaScript code, you can also check out the [real source code](TODO).

### Components are composable

Let's say we want to build a TODO. We can break this UI into a few parts:

<div className="sticky h-64 bg-red-300 top-0" />
...
Now in this Header component, it includes a custom tag `<SearchBar />`. What's in it?
...
Let's say we want to add the SearchBar into the Footer as well. What would you do?
...
### Components are live TODO

"Composable" means being able to create a component using other components as its building blocks. This sounds very similar to how nature works, except for one important difference: TODO

Try to change the SearchBar code below. What do you see?

TODO SearchBar code

The SearchBar on both Header and Footer updates. Right? Once we use a component in another component, it still "attaches" to the original component. When the original changes, all instances update along with it.
...
This works very similiarly to components in design tools such as Framer and Figma.
...
Notice that we just need to define what we want (Domo and a hat in a div), and "wire up" the data ("`type = {hat}`"). When the data change (user selects a hat), the UI updates automatically.
