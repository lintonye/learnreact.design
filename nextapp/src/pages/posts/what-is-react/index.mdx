...
import { Surprise } from '@/components/what-is-react/Surprise'
...
export const meta = {
  ...
  description: `What is React? What is it used for? Why React? This is an interactive guide for complete beginners. The only prerequisite is some knowledge of HTML.`,
  date: '2021-02-02',
  tags: ['react'],
  related: ['what-is-react-native', 'what-is-redux', 'react-useref-by-example'],
  translations: {
    简体中文: '/posts/what-is-react/zh',
    日本語: 'https://itnews.org/news_contents/what-is-react',
  },
  ogImage: {
    url: imgSocialCard,
    width: 1280,
  ...
  surprise: {
    ...
    actor: (revealEnded) => <Surprise play={revealEnded} />,
    animationVariants: {
...
What is React? What is it used for? Why React? This is an interactive guide for complete beginners. The only prerequisite is some knowledge of HTML.
...
<div className="p-8 bg-yellow-100 border border-yellow-400 rounded-sm space-y-4 my-2">
  ...
  <ConvertKitForm formId="2007263" subscribeButtonTitle="Keep Me Posted" />
</div>
...
Now let's move on to the other two superpowers of React: **declarative UI** and **reactive DOM updates**.

<div className="hidden md:block">

Have you choosen a hat for Domo yet? If not, just click one of the hats in the table. The hat on Domo's head will change, right?
...
In this UI, clicking an item in the table would update the data "selected hat". Because we have associated `DomoWithHat` with "selected hat", the hat on Domo would automatically change. Remember? **Reactive DOM updates**.

Here's the code for the component `DomoWithHat`.
...
In the code above, we just need to define what we want (Domo and a hat in a div), and "wire up" the data ("`type = {hat}`"). When the data item `hat` changes (user selects a hat), the UI updates automatically. We don't need to worry about when and how to update the DOM. We don't even need to keep track of the data. What we need is just to draw a quick sketch, i.e. the code above. Here we go, the other two superpowers of React: **Declarative UI**, plus **Reactive DOM updates**.
