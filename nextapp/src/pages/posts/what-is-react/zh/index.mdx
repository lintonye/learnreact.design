...
JavaScript？库？不是说好了这是给绝对小白的教程吗？

好吧，上面那行字是从官方网站拷过来的。接下来，让我用另一种方式为你讲解 React。很负责任地告诉你，只要对 HTML 有大致的了解（例如，HTML 标签的格式），你应该就能看懂这篇文章、理解 React 的核心概念。不懂 JavaScript？完全没问题！

具体什么方式呢？先放几个表情符提示一下：

🌴 🦅 <IconBinocular size={20} className='inline-block align-center'/>

我将带你高屋建瓴地看到 React 的概略图，而且还可以拉近、放大、看到细节，挽起袖子写一个真正的 React 组件。没错，JS 不是必需的！

准备好了吗？

<div className="p-8 bg-yellow-100 border border-yellow-400 rounded-sm space-y-4 my-2">
  <p>
    当然，要真正使用 React 的话，你还是需要写代码的。我正在编写一个 Email 邮件课程来帮助你尽快进入状态。
  </p>
  <p>
...
多年来，直接与 DOM 对话一直是 web 开发的标准方法，特别是当大多数 web 内容是静态页面的时候。程序员们典型的做法是，在静态页面的基础上点缀一些 JavaScript 代码，在页面上加一点点交互。
...
如果这几个名词听起来像天书，别担心！我会用简单的语言和插画来帮你理解。相信我，并不是那么难。
...
如果以上听起来太抽象，不要担心。我马上会使用一些例子为你详细解释。
