...
好吧，那只是官方定义。我会用另一种方式来为你讲解 React。很负责任地告诉你，只要你对 HTML 有大致的了解（例如，HTML 标签的格式），你应该就能看懂这篇文章。不懂 JavaScript？完全没问题！

本文是写给初学者的 React 简介，这也许是在学习任何 React 具体知识之前你应该读的第一篇文章。我将会用通俗的语言（和插画涂鸦 <span className='text-base'>🌴</span>）来向你讲解 React 的核心概念。

这是一幅鸟瞰图 <span className='text-2xl'>🦅</span>，但同时我也会为你装备了一副望远镜 <IconBinocular size={25} className='inline-block align-center'/>。你不仅将看到 React 的概略图，而且还可以放大、放大、看到细节，挽起袖子写一个真正的 React 组件。没错，JS 不是必需的！

准备好启航了吗？

<div className="p-8 bg-yellow-100 border border-yellow-400 rounded-sm space-y-4 my-2">
  <p>
    当然，要真正使用 React 的话，你还是需要写代码。我正在编写一个 Email 邮件课程来帮助你尽快进入状态。
  </p>
  <p>
...
我们从一个你或许听说过很多次的概念出发：DOM。

### DOM 是什么？

当你将一个网站的网址输入浏览器的时候，你的电脑和远方的另一台电脑就开始对话。这台远方的电脑一般被称之为 _服务器_。你的电脑发出请求，向服务器索取某种信息，服务器随即做出应答：
