...
Let's look at a more interesting example. What about a stop watch?

The requirement is that we want to be able to pause and resume the watch.

The watch can be implemented with `setInterval`. But we need to clear the interval when the button is clicked. And create a new interval when the watch is resumed.
