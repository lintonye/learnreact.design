...
After a lot of struggle trying to understand `useRef`, I finally saw this single sentence. And it clicked! If you are hearing the same clicking sound in your mind right now already, feel free to skip to [the examples section](TODO). Otherwise, if you are not familiar JavaScript classes or class instance variables, read on. This section is for you.

### The "re-rendering time loop"
...
This doesn't work, even though the value of `count` increases on click. Why? React is oblivious to the change of local variables and, therefore, doesn't know when to update the DOM. We'd need to re-render the component, namely request React to call the component function `Counter`, to let the change reflected on the screen.

You can verify this by adding a line `console.log('render')` right after `let count = 0`. The component isn't rendered when the user clicks the button.
...
<LiveEditor readOnly showPreview={false} showConsole={false}>
  {`
...
Why? This is what I call **_re-rendering time loop_**.

<img src={imgTimeLoop} width={787} height={415} alt="Time loop" />

Every time when the component re-renders, it goes into a time loop. All the local variables in the function will be reset to their original values. That's why `count` is always `0`.
...
As straighforward as this sounds, sometimes it's easy to forget (I do it all the time). So remember, a React component is re-rendered all the time, so all the local variables will go into a time loop, over and over again.

<div className="p-8 bg-yellow-100 border border-yellow-400 rounded-sm space-y-4 my-2">
  <p>
    BTW: if you like metaphors like this time loop, you want to check out my course: React Mental Models.
    I use metaphors and illustrations to explain the mental modals of React. For example the "re-rendering time loop" above.
...
Now what if we move the variable out of the component function? It will no longer be affected by the re-rendering time loop, right?
...
What's the point of `useRef` if it doesn't trigger a re-render when the value is updated?

In fact, being able to persist a value across re-renders without triggering another re-render is exactly why `useRef` is created. Sometimes we just don't want the component to re-render when the value is updated.
...
Look, when saving the previous state in `useEffect`, we definitely don't want the component to re-render. Otherwise, it'd go into an infinite loop. So, `useRef` to the rescue!
...
If we define `interval` in an outter scope, we'll taste the wrath of the re-rendering time loop -- `interval` will always be `undefined`.
...
How do we set and clear the timeout when the component is rendered and re-rendered, i.e. when the state `mouseDown` changes?
...
Next, how do we repeatedly increase the like count? `setInterval` is what we need. And of course we'll use another ref to keep track of the interval reference across re-renders.
...
Check out the full source code in [this CodeSandbox](https://codesandbox.io/s/useref-usepressholdrepeat-7megx)
...
We could use `ref.current` in JSX, as you've seen in [Example 1](TODO). However, keep in mind that the value displayed might be stale since updating `ref.current` doesn't trigger a re-render. We need to trigger re-renders in other means, e.g. with `useState`.
