...
`useRef` is a built-in React Hook. It is, at least in my experience, one of the most confusing Hooks (perhaps right after `useEffect`)(agree? +1 TODO). But if you spend some time to learn how it works, you'll see that it's actually a hidden gem 💎. It'll become a powerful tool in your arsenal.

I'll reveal this shiny gem <span className="mr-2">💎</span> for you in three steps: 1) what is it used for and how it works, 2) a couple of simple examples, and finally 3) a real-world example -- the like button below (click and hold for additional fun).
...
2. **Persist a value across re-renders.** This is what I'm going to focus on in this article.
...
This doesn't work, even though the value of `count` increases on click. Why? React is oblivious to the change of local variables and, therefore, doesn't know when to update the DOM. We'd need to re-render the component, namely request React to call the component function `Counter`, to notify React about the change.

You can verify this by adding a line `console.log('render')` right after `let count = 0`.
...
Now let's look at a more useful example. What about a stop watch?
