...
We could use `ref.current` in JSX, as you've seen in [Example 1](TODO). However, keep in mind that the value displayed might be stale since updating `ref.current` doesn't trigger a rerender. We need to trigger rerenders in other means, e.g. with `useState`.
